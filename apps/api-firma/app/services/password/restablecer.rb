...
module Password
  class Restablecer
    ...
    def call
      ...
      unless onboarding_token.proposito == OnboardingToken::PROPOSITO_RESTABLECER_PASSWORD
        return failure(['Token inválido'])
      end

      user = onboarding_token.user

      unless user.puede_restablecer_password?
        return failure(['No es posible restablecer la contraseña de esta cuenta'])
      end
      ...
      User.transaction do
        ...
        user.update!(debe_cambiar_password: false)
